...
class GiixCrudCode extends CrudCode {
	...
	public function generateActiveField($modelClass, $column, $htmlOptions=NULL) {
		...
		if($htmlOptions != NULL){
			...
			$htmlOptString = "array(".implode(",", $out).")";
		}
		...
		if (strtoupper($column->dbType) == 'TINYINT(1)'
			...
			return "echo \$form->textArea(\$model, '{$column->name}'".(($htmlOptString!="")?",".$htmlOptString:"").")";
		} else {
			...
			if ($column->type !== 'string' || $column->size === null)
				return "echo \$form->{$inputField}(\$model, '{$column->name}'".(($htmlOptString!="")?",".$htmlOptString:"").")";
			else{
				$out[] = "'maxlength'=>'{$column->size}'";
				$htmlOptString = "array(".implode(",", $out).")";
				return "echo \$form->{$inputField}(\$model, '{$column->name}'".(($htmlOptString!="")?",".$htmlOptString:"").")";
			}
